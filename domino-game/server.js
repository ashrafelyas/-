...
const ADMIN_KEY = process.env.ADMIN_KEY || 'change-me-1234'; // غيّرها في إعدادات الاستضافة (Environment Variables)
...
function newRoom(adminSocketId, adminName, mode, adminToken) {
  mode = MODE_CONFIG[mode] ? mode : 'teams';
  const cfg = MODE_CONFIG[mode];
  const id = nanoid(6).toUpperCase();
  const token = adminToken || nanoid(12);
  const room = {
    id,
    mode,
    playerCount: cfg.playerCount,
    createdAt: Date.now(),
    admins: new Set([token]),
  ...
  room.players[0] = { socketId: adminSocketId, name: adminName, token };
  rooms.set(id, room);
  persistRoom(room);
  return room;
}
...
// هوية دائمة (token) يولّدها المتصفح ويحفظها محليًا — تبقى ثابتة حتى لو تغيّر اتصال السوكيت
function getTokenBySocket(room, socketId) {
  const p = room.players.find(pl => pl && pl.socketId === socketId);
  if (p) return p.token;
  const s = room.spectators.find(sp => sp.socketId === socketId);
  return s ? s.token : null;
}

function isAdmin(room, socketId) {
  const token = getTokenBySocket(room, socketId);
  return !!token && room.admins.has(token);
}

function ensureLiveAdmin(room) {
  // لو ما فيه أي أدمن متصل حاليًا (نادر)، نرقّي أول لاعب متصل عشان الغرفة ما تعلق بدون أدمن
  const liveTokens = [
    ...room.players.filter(Boolean).map(p => p.token),
    ...room.spectators.map(s => s.token),
  ];
  const hasLiveAdmin = liveTokens.some(t => room.admins.has(t));
  if (!hasLiveAdmin) {
    const firstPlayer = room.players.find(Boolean);
    if (firstPlayer) room.admins.add(firstPlayer.token);
  }
}
...
function buildViewFor(room, socketId) {
  ...
  return {
    ...
    admins: [...room.admins].map(token => {
      const p = room.players.find(pl => pl && pl.token === token);
      if (p) return p.name;
      const s = room.spectators.find(sp => sp.token === token);
      return s ? s.name : 'أدمن (غير متصل حاليًا)';
    }),
    history: room.history,
...
function applyMove(room, seat, tileIndex, side) {
  ...
  if (boardEmpty) {
    room.board.push({ tile, seat, display: [tile[0], tile[1]] });
    room.leftEnd = tile[0];
    room.rightEnd = tile[1];
  } else if (side === 'left') {
    const oldLeft = room.leftEnd;
    let display;
    if (tile[0] === oldLeft) { room.leftEnd = tile[1]; display = [tile[1], tile[0]]; }
    else if (tile[1] === oldLeft) { room.leftEnd = tile[0]; display = [tile[0], tile[1]]; }
    else return false;
    room.board.unshift({ tile, seat, display });
  } else if (side === 'right') {
    const oldRight = room.rightEnd;
    let display;
    if (tile[0] === oldRight) { room.rightEnd = tile[1]; display = [tile[0], tile[1]]; }
    else if (tile[1] === oldRight) { room.rightEnd = tile[0]; display = [tile[1], tile[0]]; }
    else return false;
    room.board.push({ tile, seat, display });
  } else {
    return false;
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({ name, mode, adminKey, token }) => {
    if (adminKey !== ADMIN_KEY) {
      socket.emit('error_msg', 'مفتاح الأدمن غير صحيح — فقط صاحب اللعبة يقدر ينشئ غرفة');
      return;
    }
    const room = newRoom(socket.id, name || 'الأدمن', mode, token);
    socket.join(room.id);
  ...
  socket.on('join_room', ({ roomId, name, asSpectator, token }) => {
    const room = rooms.get((roomId || '').toUpperCase());
    if (!room) { socket.emit('error_msg', 'الغرفة غير موجودة'); return; }
    socket.join(room.id);
    socket.data.roomId = room.id;
    const myToken = token || nanoid(12);

    if (!asSpectator) {
      const emptySeat = room.players.findIndex(p => !p);
      if (emptySeat === -1) {
        room.spectators.push({ socketId: socket.id, name: name || 'مشاهد', token: myToken });
      } else {
        room.players[emptySeat] = { socketId: socket.id, name: name || `لاعب ${emptySeat + 1}`, token: myToken };
      }
    } else {
      room.spectators.push({ socketId: socket.id, name: name || 'مشاهد', token: myToken });
    }
  ...
  socket.on('promote_spectator', ({ targetName }) => {
    // الأدمن يحوّل متفرجًا معيّنًا للعب في أول مقعد فاضي
    const room = rooms.get(socket.data.roomId);
    if (!room || !isAdmin(room, socket.id)) return;

    const specIdx = room.spectators.findIndex(s => s.name === targetName);
    if (specIdx === -1) { socket.emit('error_msg', 'هذا الشخص غير موجود ضمن المتفرجين'); return; }

    const emptySeat = room.players.findIndex(p => !p);
    if (emptySeat === -1) { socket.emit('error_msg', 'لا يوجد مقعد فاضي حاليًا'); return; }

    const spec = room.spectators[specIdx];
    room.players[emptySeat] = { socketId: spec.socketId, name: spec.name, token: spec.token };
  ...
  function cleanupSocket(socket) {
    ...
    // ملاحظة: لا نحذف التوكن من room.admins هنا — هوية الأدمن تبقى محفوظة
    // وتُستعاد تلقائيًا عند عودة نفس الشخص بنفس الكود (نفس المتصفح)
    ensureLiveAdmin(room);
    broadcastRoom(room);
  }
});
