...
document.getElementById('create-room-btn').onclick = () => {
  ...
  socket.emit('create_room', { name: myName, mode, adminKey });
};
...
function joinRoom(asSpectator) {
  ...
  socket.emit('join_room', { roomId, name: myName, asSpectator });
}
...
function render(state) {
  ...
  // Seats (ديناميكي حسب عدد اللاعبين)
  const seatsEl = document.getElementById('seats');
  seatsEl.innerHTML = '';
  seatsEl.style.gridTemplateColumns = `repeat(${state.playerCount}, 1fr)`;
  for (let i = 0; i < state.playerCount; i++) {
    const p = state.players[i];
    const seatDiv = document.createElement('div');
    seatDiv.className = 'seat' + (state.status === 'playing' && state.turn === i ? ' active' : '');
  ...
  boardEl.innerHTML = '';
  state.board.forEach(entry => boardEl.appendChild(renderDomino(entry.tile, false)));
